document.addEventListener('DOMContentLoaded', () => {
    ...
    const designs = {
        'sssgruppe': {
            appName: "SSS Gruppe NFC Tool",
            short_name: "SSS",
            theme: "customer-brand",
            lockTheme: false,
            icons: { 
                icon192: "/THiXX-SSS/assets/SSS-192x192.png",
                icon512: "/THiXX-SSS/assets/SSS-512x512.png" 
            },
            brandColors: { 
                primary: "#FF4337", 
                secondary: "#ECE200" 
    ...
    async function loadTranslations() { const lang = navigator.language.split('-')[0]; const supportedLangs = ['de', 'en', 'es', 'fr']; const selectedLang = supportedLangs.includes(lang) ? lang : 'de'; const path = `/THiXX-SSS/lang/${selectedLang}.json`; try { const response = await fetch(path); if (!response.ok) throw new Error(`Language file for ${selectedLang} not found at ${path}`); appState.translations = await response.json(); document.documentElement.lang = selectedLang; } catch (error) { console.error('Could not load translations, falling back to German.', error); try { const fallbackPath = `/THiXX-SSS/lang/de.json`; const response = await fetch(fallbackPath); appState.translations = await response.json(); document.documentElement.lang = 'de'; } catch (fallbackError) { console.error('Could not load fallback German translations.', fallbackError); } } }
    ...
    // --- App Initialization ---
    async function loadConfig() { try { const response = await fetch('/THiXX-SSS/config.json'); if (!response.ok) throw new Error(`HTTP error! status: ${response.status}`); return await response.json(); } catch (error) { console.warn('Config load failed, using default.', error); return { design: "default" }; } }
    
    async function main() {
        if ('serviceWorker' in navigator) {
            window.addEventListener('load', () => {
                navigator.serviceWorker.register('/THiXX-SSS/sw.js', { scope: '/THiXX-SSS/' })
                    .then(registration => {
        ...
        // KORRIGIERT (BUGFIX): `await` hinzugefügt
    ...
    function applyConfig(config) {
        const selectedDesign = designs[config.design] || designs['sssgruppe']; // Fallback auf sssgruppe
    ...
    // --- UI/UX Functions ---
    function updateManifest(design) { const manifestLink = document.querySelector('link[rel="manifest"]'); if (!manifestLink) return; const oldHref = manifestLink.href; if (oldHref && oldHref.startsWith('blob:')) { URL.revokeObjectURL(oldHref); } const newManifest = { name: design.appName, short_name: design.short_name, start_url: "/THiXX-SSS/index.html", scope: "/THiXX-SSS/", display: "standalone", background_color: "#ffffff", theme_color: design.brandColors.primary || "#f04e37", orientation: "portrait-primary", icons: [{ src: design.icons.icon192, sizes: "192x192", type: "image/png" }, { src: design.icons.icon512, sizes: "512x512", type: "image/png" }] }; const blob = new Blob([JSON.stringify(newManifest)], { type: 'application/json' }); manifestLink.href = URL.createObjectURL(blob); }
